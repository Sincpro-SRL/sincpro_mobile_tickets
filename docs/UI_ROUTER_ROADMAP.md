...
### F0 · Higiene y red de seguridad *(habilita todo lo demás)* ✅
...
- [AUTO] ✅ Future flags de router v7 (`v7_startTransition`, `v7_relativeSplatPath`) en el `NativeRouter` del core (`AppShell.tsx`). *(Llega a las apps al republicar el core.)*
...
### F1 · System UI + Dark mode *(alto impacto visible, bajo riesgo)* 🟡
...
- [AUTO] ⬜ `useColorScheme` en el AppShell para alternar `DEFAULT_THEME`/`DEFAULT_DARK_THEME` + CSS vars `:root.dark`; `expo-status-bar` consistente; edge-to-edge SafeAreaLayout. *(Se hace tras instalar system-ui + para verificar en device.)*
- **Verificación:** toggle light/dark cambia toda la UI; prebuild sin el warning de `userInterfaceStyle`.

### F2 · Branding por contexto *(elimina una clase entera de bugs)* 🟡
...
### F3 · Layouts unificados *(arregla la raíz del blanco)* ⬜
...
### F4 · Router tipado + AuthGuard *(DX + menos boilerplate)* ⬜
...
### F5 · Config compartida + scaffolding *(escala el ecosistema)* ⬜
...
### F6 · Expo SDK latest *(plataforma al día — hacer al final, es disruptivo)* ⬜
